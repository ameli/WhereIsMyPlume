...
% block. 
    ...
    case {'2000'}  % Something from VMDAS.
      ...
      ens.pitch(k)=fread(fd,1,'int16')*cfac2;   % Pitch here (not in header?)             
      ens.roll(k)=fread(fd,1,'int16')*cfac2;     % Roll here (not in header?)             
      ens.heading(k)=fread(fd,1,'int16')*cfac2;  % Heading here (not in header?)                  
      fseek(fd,10,'cof');
      nbyte=2+76;
