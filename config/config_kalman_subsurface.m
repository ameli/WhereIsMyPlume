function Config = config_kalman_subsurface(ProjectDirectory)
    ...
    Config.ExtraTimeCutMarginIndex = 200;         % Margin before and after the begining and end of computation time CHANGED: 0 to 100
    ...
    % Velocities
    Config.ReadFromMatFiles = true;                                            % Uses Matlab mat files that exported from Teledyne's velocity software. Velocity is in earth coordinate
    ...
    % Kalman Filter
    Config.KalmanFilter.UseKalmanFilter = true;                                                 % If false, Kalman filter is not applied. If true, make sure [K1] below is also true.
    ...
    Config.SaveForPythonDirectory = fullfile(ProjectDirectry,'python','data');
