function Config = config_subsurface(ProjectDirectory)
    ...
    % Config.TimeShift = 4;          % In hours, This is added to Data.Time which will be in UTC time. Use 4 hours when using velocity from mat files.
    Config.TimeShift = 7 + 53/60;    % In hours, This is added to Data.Time which will be in UTC time. Use 7:53' when using velocity from LTA files
    ...
    Config.PlumeRelease.ReleaseDepth = 15;   % Meters. Positive downward. (3m surface, 15m subsurface)  % CHANGED: 3 to 2.88
    ...
    Config.PlumeLastSeen.LastObservedTime = [2018, 08, 16, 11, 40, 01];                       % [B1]
    Config.PlumeLastSeen.UseDrifterPositionAtObservedTimeForPlume = true;                     % [B2] If true, it uses drifter poisition, set [B3]
    Config.PlumeLastSeen.DrifterIdsToUseForPositionAtObservedTimeForPlume = [19];             % [B3] Set if [B2] is true
    ...
    Config.PlumeInquiry.InquiryTime = [2018, 08, 16, 17, 40, 00];         % [A1] Surface release
    ...
    % Velocities
    Config.ReadFromMatFiles = false;                                           % Uses mat files exported from Velocity software. if true, set OceanVelocityIsRelativeToShip=false, and set TimeShift = 4.
    ...
    % Windage
    Config.Wind.IncludeWindData = false;
    ...
    Config.Kriging.SpatialBasisFunctionPolynomialDegree = 2;                  % Polynomial degree for only spatial variables x and y.
    ...
    Config.Kriging.SplineStifnessInDualKriging = 0;                           % Spline stiffness weight
    ...
    Config.Kriging.TemporalDecorrelationScale  = [0 0 0 00 30 00];            % In the unit of yyyy,mm,dd,HH,MM,SS
    ...
    Config.Kriging.ShipTimeIndexSkipping = 1;                                 % Index. 1 reads all ship positions. 2 reads every 2 index, and so on.
